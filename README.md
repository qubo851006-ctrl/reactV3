...
- **三层防护**：
  1. **CI `security-audit` job**：`pip-audit -r requirements.txt`（后端生产依赖 vs PyPI/OSV 漏洞库）+ `npm audit --omit=dev --audit-level=high`（前端生产依赖）。**报告模式**（见下）：漏洞打印进 job 日志，不判红构建。
...
- **范围决策**：
  ...
  - **报告模式（`continue-on-error`）而非硬门禁**：首次接扫描器时仓库已有**历史 CVE 欠账**，这些要么需大版本升级、要么是传递依赖，不能盲目自动改；让欠账长期把 master 判红会训练大家无视告警。所以现阶段漏洞照常打印进日志 + Dependabot 照常开修复 PR，但不判红构建。上线前清理欠账后，去掉 ci.yml 两个审计步骤的 `continue-on-error` 即变硬门禁。
- **现状**：前端生产依赖 `npm audit` 实测 **0 漏洞**；后端 `pip-audit` 抓到历史欠账：
  - `pillow==11.1.0` —— 多个 2026 CVE（CVE-2026-25990 / CVE-2026-40192 / PYSEC-2026-165），修复在 12.x（大版本，升级后需回归图像/扫描件识别）。
  - `pdfminer-six`（经 `pdfplumber==0.11.4` 传递引入）—— CVE-2025-64512 / CVE-2025-70559。
  - **这些是历史欠账，上线前需集中评估升级**（见 `docs/CI.md` 的"已知欠账"清单）。
...
- **做法**：新增 `tests/test_excel_merger.py`，**30 个单测**，覆盖率 **7% → 99%**：
  - 纯函数：编号归一化（大小写 / 全角括号 / 末尾"号"）、表头清洗、表头行探测、合同编号索引、**匹配状态 8 个分支**（全匹配 / 部分匹配 / 全不匹配 × 上传组合）；
  - IO 管线：表头行探测、合计行 / 空行跳过、"汇总" Sheet 优先、缺"合同编号"列报错、`*` 星号表头清洗；
  - 主管线 `merge_ledgers`：匹配统计计数、跨格式模糊匹配、**一对多笛卡尔积展开**、完全一致行去重、优先级排序、输出列含三表前缀——均通过**回读输出 Excel** 验证。
- **零风险**：纯测试新增，未改动任何生产代码。后端 pytest **425 全过**、ruff 全绿。`docs/COVERAGE.md` 已更新基线（excel_merger 标记为已补）。
- 对应 2026-06-02 工程性再审计健壮性清单 #5。

---

## 2026-06-03 更新（v3.6.17 · 前端错误边界 ErrorBoundary）

- **背景**：此前前端**无 ErrorBoundary**——任一组件在渲染期/生命周期抛异常，React 会卸载整棵组件树，导致**整个应用白屏**，用户只能强刷且丢失上下文。
- **做法**：新增可复用 `frontend/src/components/ErrorBoundary.tsx`（类组件，React 未提供 Hook 版），两层防护：
  - **顶层**（`main.tsx` 包裹 `<App/>`，`fullPage` 全屏兜底）——白屏的最后一道防线，渲染"应用遇到错误 + 刷新页面"。
  - **区域级**（六个业务 Flow：培训 / 案件台账 / 授权请示 / 三台账合并 / 审计分析 / 合规审查，各自包内联边界）——单个模块崩溃只显示内联卡片并隔离，侧栏 / 头部 / 其它会话仍可正常使用。
- **兜底体验**：内联卡片含错误信息 + "重试 / 刷新页面"；点"重试"即重挂该模块（无需刷新整页），Flow 名由技能注册表 `SKILLS` 派生（如"三台账合并出错了"）。错误同时 `console.error` 记录，后续可接入前端日志上报。
- **测试**：新增 `tests/ErrorBoundary.test.tsx` 6 个用例（正常透传 children / 内联兜底含 label 与错误信息 / 全屏兜底 / 点重试恢复 / 默认 label / 自定义 fallback 优先）。前端 vitest **47 全过**、eslint 干净、production build 通过。
- 对应 2026-06-02 工程性再审计健壮性清单 #2（防白屏，投入小而独立）。
...
- **后端 `/api/admin/audit-logs` 接口增强**：补齐 `user_name`（批量 join `users` 避免 N+1）、`target_type` / `target_id` 字段；新增 `action` 关键词筛选和 `limit` 参数（上限 500）；仍要求管理员权限。
- **测试**：后端新增 4 个用例（降序+user_name join / action 筛选 / limit / 非 admin 403），前端新增 2 个用例（audit tab 展示 / action 筛选），后端 pytest 371 全过、前端 vitest 41 全过。
- 对应 2026-05-29 健壮性体检报告优化清单的 #9。
...
- **基线**：后端业务代码 **63%**、前端 src **30%**（详见 `docs/COVERAGE.md`）。
- **看清盲区**：`utils/excel_merger.py`（三台账合并核心计算逻辑）仅 **7%** 覆盖、最该补；`mcp_client` / `write_excel` / `pdf_reader` / `classifier` 等旧工具 0%；前端多数业务组件靠 e2e 导航冒烟兜底、无单测。
- **新增 `docs/COVERAGE.md`**：记录基线、运行方法、已知盲区与补测优先级。
- **定位**：本次只把"统计能力"建起来 + 记录基线，作为后续重构（大文件拆分 / App.tsx 拆路由）的安全网；当前不设 CI 强制阈值（求快稳），未来可加 `--cov-fail-under` 软门禁防倒退。
- 对应 2026-05-29 健壮性体检报告优化清单的 #8。
